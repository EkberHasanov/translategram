...
async def mtranslate_translate_str_returns_str_test(mtranslate_service) -> None:
    ...
    target_language = 'es'
    source_language = 'en'
    result = await mtranslate_service.translate_str(text, target_language, source_language)
    assert isinstance(result, str)


async def mtranslate_translate_str_without_source_returns_str_test(mtranslate_service) -> None:
    text = "Hello World!"
    target_language = 'es'
...
async def mtranslate_translate_str_raises_error_on_invalid_input_test(mtranslate_service):
    with pytest.raises(TypeError) as exc_info:
        await mtranslate_service.translate_str(123, 'en')
    assert str(exc_info.value) == "`text`, `target_language` and `source_language` must be a string"
...
async def mtranslate_translate_str_handles_empty_string_test(mtranslate_service) -> None:
    result = await mtranslate_service.translate_str('', 'es')
    assert result == ''


async def mtranslate_translate_str_handles_special_characters_test(mtranslate_service) -> None:
...
async def mtranslate_translate_str_performance_test(mtranslate_service):
    ...
    assert avg_elapsed_time < performance_threshold, \
